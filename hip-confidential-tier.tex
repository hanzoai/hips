...
environment's X-Wing public key, $pk_r$ its receipt verifying key, $\mathrm{sid}$
the session identifier, and $n_v$ a nonce the verifier generated.
...
\begin{description}[leftmargin=1.6em]
\item[\req{V1}] Recompute $\mathrm{rd}$ from the bundle's own \code{launch},
\code{workload}, \code{ke}, \code{kr}, \code{sid} and \code{nonce} per \req{B1},
and require it to equal the report's report-data field. A report whose caller field
says something else is a report about a different environment.
...
\begin{description}[leftmargin=1.4em]
\item[Environment.] The measured image. Conforms if it satisfies
\req{B1}--\req{B5}, \req{B7}--\req{B8}, \req{E1}--\req{E3}, \req{K3} and
\req{R1}--\req{R3}.
...
\item[\req{X2}] There MUST be one attester and one verifier. \emph{Observed:} three
places could hold them today. The mocks in \code{hanzo-pqc} and \code{hanzo-kbs}
return success for any input --- one accepts any quote against any expected
measurement, the other accepts any non-empty report as an SEV-SNP attestation at
level 2. \code{hanzo-attest} in \code{hanzoai/node} declares the two hardware roots
and refuses them by name. \emph{Proposed:} the implementation belongs where refusal
already is, because refusing by name is the correct behaviour for an unimplemented
root and returning success is not. The mocks are removed at the same time, not
deprecated.
...
to simulation \\
\req{V0}--\req{V15} & \code{hanzo-pqc}, \code{hanzo-kbs} & absent & Mocks return
success for any input \\
