...
`/v1/auto` is workflows that run themselves, on a schedule or a webhook. An org
authors a flow — a trigger and a tree of connector actions — and this runs it
durably and keeps its run history. It composes three things it does not own:
per-org connector credentials, the one shared durable engine, and the one tenant
gate. It is implemented in `hanzoai/cloud` at `apps/auto` (HIP-0106).
...
through the one opener; the sub name is the file key the content encryption
derives from, so it keeps the package's former spelling and a reader should
expect that mismatch rather than read it as drift — holding four tables: flows, their versions, their runs,
...
- **flow** is an agent workflow on a visual canvas: a person composes a graph of
  ...
  in a session.
- **auto** is the trigger→action tree that fires with nobody there: a cron
  tick, an inbound provider event, an internal producer. Nobody presses go,
  which is the whole of the distinction.
- **tasks** (HIP-1062) is the durable engine as a product — a customer's own
  workflow, made to survive a crash and replayable. auto is one workflow family
  running on it. Every flow run is a durable run; not every durable run
  is a flow.
...
- **integrations** (HIP-1250) holds the org's connected accounts. auto holds
  what to do with them. Connecting an account and using one are separate
...
- `/v1/auto/connectors` — the connector catalogue: each external service a step
  ...
  triggers. It is compiled into the binary and its schema is a build-time
  contract, so a mismatch fails the build rather than the first request. The
  catalogue is identical for every tenant, so the gate is a validated principal
  rather than a per-org view. `/v1/auto/pieces` answers the
...
> A typed operation MUST be addressable through its input alone, because on two
> of its four transports the input is the whole message.

Over the tool plane and the internal call plane the arguments object *is* the
request and no path is bound from it, so an `In` whose own decoder swallows the
body keeps the REST wire intact and silently makes the operation unaddressable
everywhere else.
...
The surface declares `cloud.Metered` (`plugin/auto/main.go`).

One unit is metered: a run start, kind `automations.run`, priced from the
deployment's fee (`CLOUD_AUTO_FEE_CENTS`), zero meaning free. The kind, the
durable queue name and the ledger product keep the former spelling on purpose: a
running workflow polls the queue it was started on, and a customer's spend is
read back by the product label already written beside every debit. A tool call
...
It publishes nothing on the bus: no `auto.*` event reaches a customer's
...
connector, which is where the amplification is easiest to see and hardest to
keep correct: every new connector would re-derive the same ceilings, and the one
that forgot would be the incident. Depth, dedupe and the per-org budgets sit at
dispatch, so a connector cannot opt out of them by existing.
