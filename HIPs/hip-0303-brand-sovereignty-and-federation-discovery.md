...
Hanzo source trees MUST NOT contain references to Liquidity (the regulated US securities ATS at `/*`). Specifically: zero substring matches for ``, `@/*`, or any Liquidity surface name in Hanzo source. Hanzo Inc. and Liquidity LLC have distinct legal and compliance scopes; source-level mention blurs that boundary.

The inverse rule (Liquidity must not reference Hanzo) is codified in `/*` CLAUDE.md.
